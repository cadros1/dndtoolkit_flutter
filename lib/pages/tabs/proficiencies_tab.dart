...
class _ProficienciesTabState extends State<ProficienciesTab> {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (constraints.maxWidth >= 500) {
          // 将 5 个分组拆成两列：左列 3 个，右列 2 个
          return Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Expanded(
                child: Column(
                  children: _skillsData.sublist(0, 3).map((d) => _buildSkillGroupWidget(d.name, d.skills)).toList(),
                ),
              ),
              const SizedBox(width: 16),
              Expanded(
                child: Column(
                  children: _skillsData.sublist(3, 5).map((d) => _buildSkillGroupWidget(d.name, d.skills)).toList(),
                ),
              ),
            ],
          );
        ...
        // 窄屏：垂直堆叠
        return Column(
          children: _skillsData.map((d) => _buildSkillGroupWidget(d.name, d.skills)).toList(),
        );
      },
    );
  }
  ...
  Widget _buildCheckItem(String label, bool value, Function(bool) onChanged) {
    ...
    final itemWidth = min(screenWidth / 2 - 20, 340.0);
    return SizedBox(
      width: itemWidth,
