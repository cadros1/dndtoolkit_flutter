...
项目采用轻量的 Flutter 单体应用结构：Widget 直接持有页面状态和可变的 `Character` 对象，服务类负责文件、云端、PDF、令牌和更新等边界能力。当前没有独立 repository 层、依赖注入容器或全局状态管理框架。
...
骰子日志只存在于当前 `AdventurePage` 实例内，不进入 `Character`，也不持久化。
