...
| `lib/models/dm_models.dart` | NPC 卡、分类、遭遇预设、实例快照、集群和唯一当前遭遇模型 |
| `lib/pages/` | 页面、路由和页面内状态 |
| `lib/pages/dm/` | DM 任务导航、NPC 库/编辑器、分类管理、遭遇预设和当前遭遇 UI |
...
dm/dm_data.json → DmStorage → DmData
  → NPC 库创建/编辑模板与分类
  → 遭遇预设保存卡片快照、数量、实例名称与集群安排
  → 从预设创建遭遇，或从当前遭遇提取预设草稿
  → 实例化时复制 CardSnapshot
  → 当前遭遇修改 HP / 临时 HP / 先攻 / 集群
  → 每次操作及页面/应用生命周期保存
```

DM 通用掷骰日志只存在于当前 `CurrentEncounterView` 页面会话，不进入 `DmData`。当前遭遇始终只有一个；NPC 模板、预设、实例和集群不复用 `Character` JSON，也不进入现有 Supabase `characters` 表。
