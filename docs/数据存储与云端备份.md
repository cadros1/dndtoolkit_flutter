...
该文件根对象以 `DnDToolkit-DM: 3` 标识内部 schema，并同时保存 NPC 分类、NPC 卡、遭遇预设和唯一当前遭遇。版本 1、2 文件仍可读取；旧版加载时预设列表为空，并补齐不可删除的默认分类、按属性值重算六维调整值。实例和预设条目都包含创建时的完整 `CardSnapshot`，因此模板后续修改或删除不会影响当前遭遇或已有预设。DM 读写在所有页面实例间串行执行，保存先写同目录临时文件再替换正式文件，避免响应式页面重建时读取半写入 JSON。DM 数据目前只保存在本地，不进入角色云备份或 PDF。Markdown 只交换用户明确导出的单张 NPC 卡模板，不是 `dm_data.json` 的副本，也不包含分类 ID、预设、实例、动态 HP、先攻、集群或遭遇状态。解析失败时 DM 页面提示并保留原文件。
