...
AI 服务配置不进入角色 JSON，也不上传到 Supabase。配置元数据以 UUID 为主键保存在 `SharedPreferences`，包括配置名、服务类型、Base URL、模型和思考设置。API Key 按配置 UUID 保存在 `flutter_secure_storage`，编辑时留空表示保留，删除配置时同步删除。
