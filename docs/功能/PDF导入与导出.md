...
4. 文件名来自角色名，并将 Windows 禁止字符替换为 `_`；空名使用 `Unnamed_Character.pdf`。Android 分享文件会追加精确到微秒的本地导出时间，例如 `角色名_20260813_140509_023007.pdf`，避免 QQ 等接收端因同名文件自行追加不连续的 `(n)` 后缀；分享结束后删除应用侧源临时文件。
...
- Android 分享测试应覆盖同一角色连续导出时文件名唯一、PDF MIME 类型明确，以及应用侧源临时文件清理。
